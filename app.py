...
def check_password():
    ...
    correct_password = os.environ.get("APP_PASSWORD", "DefaultPassword123!")
...
with st.sidebar:
    ...
    # Tab-basierte Auswahl
    tab1, tab2 = st.tabs(["📁 Ordner", "📤 Upload"])
    
    with tab1:
        # Default: User's Academy folder locally, documents folder on cloud  
        user_academy_path = os.path.expanduser("~/Documents/Academy")
        if os.path.exists(user_academy_path):
            default_docs_dir = os.environ.get("DOCS_DIR", user_academy_path)
        else:
    ...
    with tab2:
        st.info("📤 Dokumente hochladen")
        
        # Option 1: Mehrere Dateien gleichzeitig
        uploaded_files = st.file_uploader(
            "Wähle Markdown-Dateien aus",
            type=['md'],
            accept_multiple_files=True,
            help="💡 Tipp: Wähle mehrere .md Dateien gleichzeitig aus (Strg+Klick oder Cmd+Klick)"
        )
        
        st.markdown("**📁 Ordner hochladen:**")
        st.markdown("""
        Da Browser keinen direkten Ordner-Upload unterstützen, gibt es mehrere Möglichkeiten:
        
        **Option A: Alle Dateien markieren**
        1. Öffne deinen Ordner im Datei-Explorer
        2. Markiere alle .md Dateien (Strg+A oder Cmd+A)
        3. Ziehe sie in das Upload-Feld oben
        
        **Option B: ZIP-Datei hochladen**
        1. Packe deinen Ordner in eine .zip Datei
        2. Lade die ZIP-Datei hier hoch:
        """)
        
        # Option 2: ZIP Upload
        uploaded_zip = st.file_uploader(
            "ZIP-Datei mit Markdown-Dateien hochladen",
            type=['zip'],
            help="Packe deine .md Dateien in eine ZIP-Datei"
        )
        
        # Verarbeitung für normale Dateien
        if uploaded_files:
            if st.button("📚 Hochgeladene Dateien indexieren", key="process_files"):
                documents = process_uploaded_files(uploaded_files, "Einzelne Dateien")
                if documents:
                    finalize_document_processing(documents)
        
        # Verarbeitung für ZIP-Dateien  
        if uploaded_zip:
            if st.button("📚 ZIP-Datei verarbeiten", key="process_zip"):
                documents = process_zip_file(uploaded_zip)
                if documents:
                    finalize_document_processing(documents)
        
        if not uploaded_files and not uploaded_zip:
            st.write("Noch keine Dateien hochgeladen")
